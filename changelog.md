...
- Added new user configuration options and guidance for selecting an optimization solver and setting a maximum time limit for finding a solution.
- Created a SAF-specific Scenario Setup Template to guide users through creating a FTOT scenario focused on a regional SAF supply chain. The Excel-based template contains step-by-step instructions on how to define the supply chain geography, feedstock supply, and SAF pathway, in addition to choosing blending facilities and airport destinations. The Scenario Setup Conversion Tool has been expanded to be compatible with the SAF-specific template. Data sources can be found in the Technical Documentation. Detailed instructions can be found in the template itself.
- Refined the unmet demand penalty (UDP) sensitivity analysis tool to handle unexpected runtime issues by re-running iterations starting from the S step. Additional logging outputs have been added in the command prompt, showing the iteration step with the best solution.
- Other updates:
