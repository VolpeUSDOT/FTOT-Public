...
def create_constraint_daily_processor_capacity(logger, the_scenario, prob, flow_var, processor_build_vars,
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        for row_a in processor_facilities:
            ...
            max_capacity = facility_cap_noEarthquake[float(facility_id)-1][t+1][i]*row_a[2]
            day = row_a[3]
...
def create_constraint_max_route_capacity(logger, the_scenario, prob, flow_var):
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        for key, flow in iteritems(flow_lists):
            prob += lpSum(flow) <= key[1], "constraint max flow on nx edge {} commodity {} for day {}".format(key[0],  key[2], key[3])
...
def save_pulp_solution(the_scenario, prob, logger, zero_threshold=0.00001):
    ...
    with sqlite3.connect(the_scenario.main_db) as db_con:
        ...
        if optimal_unmet_demand_sum < 0:
            costs_yearly = (abs(float(value(prob.objective))- optimal_unmet_demand_sum * the_scenario.unMetDemandPenalty))*((1+Inflation_rate)**t) + sum(CatalystReplace_cost[:,t+1,i])

            logger.result(
                "Total Scenario Cost = (transportation + processor construction + operation + catalyst replacement): \t ${0:,.0f}"
                "".format(costs_yearly))
        else:
            costs_yearly = (abs(float(value(prob.objective))-optimal_unmet_demand_sum * the_scenario.unMetDemandPenalty))*((1+Inflation_rate)**t)+sum(CatalystReplace_cost[:,t+1,i])+(optimal_unmet_demand_sum*the_scenario.unMetDemandPenalty)*((1+Inflation_rate)**t)
            logger.result(
