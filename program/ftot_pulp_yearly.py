...
# initial costs (yearly)
# run optimization without considering any risk factors
...
def generate_all_vertices(the_scenario, logger):
    ...
    logger.info("facility capacity index {} for scenario {}".format(facility_cap_noEarthquake[:,t+1,i],i))
    # ===============================================================================
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        for row_a in facility_data:
            ...
            if facility_type == "processor":
                logger.info("facilities id {}".format(facility_id))
               ...
               and mc.commodity_name = '{}';""".format(facility_id, multi_commodity_name))
                ...
                for row_b in proc_data:
                    ...
                    #====================================Modification==========================================================
                    # Following part is modified to include time-varying facility capacity due to technology development scenario
                    # Quantity * year for yearly simulation
                    quantity = facility_cap_noEarthquake[facility_id-2][t+1][i]*year*row_b[1]         
                    #======================================End==================================================================
                    # units = row_b[2]
            ...
            # if we bring storage facilities back (12-2018), we would want them split by source_facility
                ...
                for row_b in rmp_data:
                    commodity_id = row_b[0]
                    #====================================Modification==========================================================
                    # Following part is modified to include time-varying facility capacity due to climate scenario
                    # Quantity * year for yearly simulation
                    quantity = facility_cap_noEarthquake[facility_id-1][t+1][i]*year*row_b[1]
                    logger.info("rmp facilities quantity {}".format(quantity))
                    #====================================End==================================================================
                    # units = row_b[2]
...
def create_opt_problem(logger, the_scenario, unmet_demand_vars, flow_vars, processor_build_vars):
 ...
 #===============================================Modification==========================================================
    # Add operation cost calculation into objective
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        for row_a in processor_facilities:
            ...
            quantity = facility_cap_noEarthquake[facility_id-2][t+1][i]*year*row_a[3]
            #quantity = row_a[3]
            day = row_a[4]
    ...
    prob +=(initial_cost_yearly + weight*(lpSum(unmet_demand_costs) + lpSum(flow_costs[k] * flow_vars[k] for k in flow_costs) + lpSum(
        processor_build_costs)+ lpSum(operation_cost) - lpSum(non_operation_cost) - initial_cost_yearly)), "Total Cost of Transport, storage, facility building, operation and penalties"
...
def create_constraint_daily_processor_capacity(logger, the_scenario, prob, flow_var, processor_build_vars,
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        for row_a in processor_facilities:
            ...
            #====================================Modification==========================================================
            # Following part is modified to include time-varying facility capacity due to technology development scenario
            # Quantity/capacity * year for yearly simulation
            max_capacity = facility_cap_noEarthquake[facility_id-2][t+1][i]*year*row_a[3]
            #==================================End=====================================================================
...
def create_primary_processor_vertex_constraints(logger, the_scenario, prob, flow_var):
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        for row_a in sql_data:
            ...
            #====================================Modification==========================================================
            # Quantity/capacity * year for yearly simulation
            quantity = facility_cap_noEarthquake[facility_id-2][t+1][i]*year*float(row_a[7])
            #==================================End=====================================================================
...
def create_constraint_max_route_capacity(logger, the_scenario, prob, flow_var):
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        for row_a in storage_edge_data:
            route_id = row_a[0]
            aggregate_storage_capac = 0.2*row_a[1]
...
def solve_pulp_problem(prob_final, the_scenario, logger):
    ...
    logger.result(
        "Total Scenario Cost = (transportation + unmet demand penalty + processor construction + operation): \t ${0:,.0f}".format(
...
def save_pulp_solution(the_scenario, prob, logger):
    ...
    logger.info("scenario {}".format(i))
    logger.info("time horizon {}".format(t))
    #logger.info("unmet_demand_yearly: {}".format(unmet_demand_yearly))
    ...
    with sqlite3.connect(the_scenario.main_db) as db_con:
        ...
        logger.info("number of solution variables greater non zero: {}".format(non_zero_variable_count))
        sql = """
        ...
        logger.info("Total Cost of resilience : \t ${0:,.0f}".format(
            float(value(prob.objective)) - initial_cost_yearly))
        #=================================non consider the UDP when UD < 0============================
        if optimal_unmet_demand_sum < 0:
            costs_yearly = float(value(prob.objective))- optimal_unmet_demand_sum * the_scenario.unMetDemandPenalty
            logger.result(
                "Total Scenario Cost = initial cost + weight * resilience cost: \t ${0:,.0f}"
                "".format(float(value(prob.objective))- optimal_unmet_demand_sum * the_scenario.unMetDemandPenalty))
        else:
            costs_yearly = float(value(prob.objective))
            logger.result(
                "Total Scenario Cost = initial cost + weight * resilience cost: \t ${0:,.0f}"
