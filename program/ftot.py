...
# load the total repair weeks
total_repair_week = np.load("total_repair_week.npy")
...
if __name__ == '__main__':
    ...
    try:
        ...
        # optimization
            ...
            #=================================================Modification================================================================
            # Purpose of following part: (a)iterate optimization per week;(b)incorporate resilience calculation for each scenario

            # Variables for resilience calculation:
            UnmetDemandAmount = np.zeros((total_repair_week+1))
            WeeklyCost = np.zeros((total_repair_week+1))
            production = np.zeros((total_repair_week+1))
                                  
            Resilience_hazard = np.zeros((total_repair_week+1))# resilience array in terms of hazard events

            # for each week:
            for j in range(total_repair_week+1):
                # earthquake occurs at the begining of second week, so we have "total_repair_week+1" iterations 
                earthquake_week = j
                #temp_day = int(7*j)
                ...
                # after optimization, load results
                UnmetDemandAmount[j] = np.load("unmet_demand_weekly.npy")
                WeeklyCost[j] = np.load("costs_weekly.npy")
                production[j] = np.load("production.npy")
                           
                # Calculate initial weekly costs
                weekly_cost = WeeklyCost[j]  
                initial_cost = WeeklyCost[0]
                
                Resilience_hazard [j] = (float(weekly_cost-initial_cost)/float(production[j]))
                 
                logger.info("Hazard-induced Resilience for {} week: {}".format(j,Resilience_hazard[j]))
                np.save("Resilience_hazard.npy", Resilience_hazard)

            Resilience = np.sum(Resilience_hazard[:])
            logger.info("Supply Chain Resilience: {}".format(Resilience))                  
