...
# we have the user specify a old XML that needs updating.
# this could be user input from the cli, and as easy as droping a file in the window. 

# load both documents to the lxml etree
# iterate through the template
#     for each element in template
#      loop through the old XML document
#     if there is a match, set the template to what text/attribute/whatever in the old XML
#     if there is no match as the end of the loop, query the user for values.
#     continue until all the elements are set.
#
...
# TODOS - FUTURE TOOL IMPROVEMENTS
# find and replace text; e.g. replace an old path from a branch to the trunk
#
...
def do_the_upgrades(the_template_etree, the_old_xml_etree):
    ...
    for temp_elem in the_template_etree.getiterator():
        ...
        #print("LOOP #{} -- template element: {}".format(item_counter, clean_temp_elem))
        #print("LOOP #{} -- text: {}".format(item_counter, temp_elem.text))
