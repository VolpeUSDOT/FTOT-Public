...
def prepare_tableau_assets(timestamp_directory, the_scenario, logger):
    """
    Prepare assets for the Tableau dashboard and package them into a .twbx file.

    This function creates a File Geodatabase (GDB) formatted for Tableau, containing merged
    facility data and optimized route segments. It also copies relevant CSV reports
    (generated by other functions) and the Tableau workbook template, then zips them
    into a packaged workbook.

    :param timestamp_directory: Path to the directory for the current run's reports.
    :type timestamp_directory: str
    :param the_scenario: The scenario object containing configuration and paths.
    :type the_scenario: ftot_scenario.FtotScenario
    :param logger: Logger instance for recording progress.
...
def generate_edges_from_routes_summary(timestamp_directory, the_scenario, logger):
    """
    Generate a summary of routes generated with NDR and export to CSV and DB.

    This function aggregates route data, including costs, lengths, emissions, and
    optimal solution status. It handles logic for artificial links (subtracting them if configured)
    and unit conversions.

    :param timestamp_directory: Path to the directory where the CSV report will be saved.
    :type timestamp_directory: str
    ...
    :db_reads: route_reference, facilities, facility_type_id, networkx_edge_costs, commodities,
               route_edges, networkx_edges, optimal_route_segments
    :db_writes: all_routes_results
    """
...
def generate_cost_breakdown_summary(timestamp_directory, the_scenario, logger):
    """
    Calculate cost breakdowns by category and export to CSV and DB.

    Analyzes `optimal_route_segments` to categorize costs.
    ...
    :db_reads: networkx_edges, optimal_route_segments, commodities, optimal_scenario_results,
               facilities, facility_type_id, facility_commodities
    :db_writes: costs_results
    """
...
def generate_artificial_link_summary(timestamp_directory, the_scenario, logger):
    """
    Generate statistics for artificial links and export to CSV and DB.

    Calculates length for all artificial links. Calculates transport cost, routing cost, access cost, network used,
    CO2 emissions, vehicle-distance traveled, and fuel burn for artificial links used in the optimal solution.
    ...
    :db_reads: facilities, facility_type_id, networkx_nodes, networkx_edges, optimal_route_segments
    :db_writes: artificial_link_results
    """
...
def generate_detailed_emissions_summary(timestamp_directory, the_scenario, logger):
    """
    Export detailed emissions data to a CSV report.

    Queries the `detailed_emissions` table and writes the content to a CSV file.
...
def generate_reports(the_scenario, logger):
    """
    Orchestrate the generation of all FTOT reports.

    Parses log files to create a human-readable text report (`report_*.txt`).
    Generates a main results CSV (`report_*.csv`) summarizing scenario results
    and configuration. Finally, calls sub-functions to
    generate other specific reports (routes, artificial links, emissions, costs, Tableau).
