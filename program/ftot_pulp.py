...
def o1(the_scenario, logger):
    """
    Orchestrates the optimization problem setup.

    This function serves as a wrapper to initialize the optimization problem
    by creating vertices, edges, and setting capacities based on the scenario configuration.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object for recording progress and errors.
    :return: None
    """
...
def o2(the_scenario, logger):
    """
    Orchestrate the creation and solving of the optimization problem and recording of optimal flows.

    This function creates the PuLP variables and constraints, solves the linear
    programming problem using the configured solver, and saves the results back to the database.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object to record progress and debug info.
    :return: None
    """
...
def generate_schedules(the_scenario, logger):
    """
    Read schedule data from the database and parse it into a dictionary of facility availability.

    Reads the `schedules` table to determine facility availability on specific days.
    Handles default availabilities and day-specific overrides. [explain input file format?]

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object.
    :return: A tuple containing:
             - schedule_dict (dict): Keys are schedule_ids, values are lists of availability floats per day.
             - last_day (int): The total duration (number of days) of the schedule.
    :db_reads: schedules
    """
...
def source_tracking_setup(the_scenario, logger):
    """
    Configure source tracking for commodities that require lineage preservation.

    This function populates the `source_commodity_ref` table. It identifies commodities
    that must track their original production facility (e.g., specific crude types)
    and handles the inheritance of this tracking through processors.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object.
    :return: None
    :db_reads: facilities, facility_commodities, commodities, facility_type_id
    :db_writes: commodities (insert multicommodity), source_commodity_ref
    """
...
def generate_all_vertices(the_scenario, schedule_dict, schedule_length, logger):
    """
    Generate graph vertices for all facilities over the scenario timeframe.

    Creates the `vertices` table in the database. Vertices represent the state of a facility
    at a specific location, for a specific commodity, on a specific day.
    Differentiates between 'primary' vertices (processing/production) and 'storage' vertices.

    :param the_scenario: The configuration object containing settings and paths.
    :param schedule_dict: Dictionary of schedule availabilities generated by `generate_schedules`.
    :param schedule_length: Integer length of the simulation schedule.
    :param logger: The logging object.
    :return: None
    :db_reads: facilities, facility_type_id, facility_commodities, commodities, source_commodity_ref
    :db_writes: vertices, networkx_nodes (index), networkx_edges (index)
    """
        ...
        for row_a in facility_data:
            ...
            if facility_type == "processor":
                ...
                for row_b in storage_fac_data:
                    ...
                    main_db_con.executemany(
                        """insert or ignore into vertices (
                       location_id, facility_id, facility_type_id, facility_name, schedule_day, commodity_id,
                       storage_vertex,
                            source_facility_id, iob)
                       values (?, ?, ?, ?, ?, ?, ?,
                ...
                for row_b in dest_data:
                    ...
                    storage_vertex_rows = [
                        ...
                        for day_before, _availability in enumerate(schedule_dict[schedule_id]) 
                        # scheduled availability isn't used for storage vertex. _availability isn't needed
                        # this query was previously part of a for loop with the query above
                    ]
                    main_db_con.executemany(
                        """insert or ignore into vertices (
                       location_id, facility_id, facility_type_id, facility_name, schedule_day, commodity_id,
...
def add_storage_routes(the_scenario, logger):
    """
    Define internal routes representing onsite storage capability.

    Creates routes that connect primary facility vertices to their storage counterparts,
    and storage vertices across subsequent days. Populates `storage_routes` and
    `route_reference` tables.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object.
    :return: None
    :db_reads: facilities, networkx_edge_costs (via assumption for defaults)
    :db_writes: storage_routes, route_reference
    """
...
def generate_connector_and_storage_edges(the_scenario, logger):
    """
    Generate graph edges representing storage and facility connections.

    Creates 'storage' edges (time travel at same location) and 'connector' edges
    (linking primary vertices to storage vertices) in the `edges` table.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object.
    :return: None
    :db_reads: commodities, vertices, storage_routes, route_reference, facility_commodities, pipeline_mapping (optional), capacity_nodes (optional)
    :db_writes: edges, pipeline_mapping (index), capacity_nodes (index)
    """
...
def generate_first_edges_from_source_facilities(the_scenario, schedule_length, logger):
    """
    Generate initial transport edges originating from source facilities.

    This identifies transport links connecting Raw Material Producers (RMPs) or other source
    facilities to the network graph, constrained by schedule duration and permitted modes.
    This step prepares the 'transport' edges in the `edges` table for subsequent expansion.

    :param the_scenario: The configuration object containing settings and paths.
    :param schedule_length: Integer length of the simulation schedule.
    :param logger: The logging object.
    :return: None
    :db_reads: commodity_mode, networkx_edges, networkx_nodes, networkx_edge_costs, vertices, facility_type_id, facility_commodities, pipeline_mapping
    :db_writes: edges
    """
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        left outer join (--get facilities with incoming transport edges with tracked distance
        select vi.facility_id, min(ei.distance_travelled) distance_travelled, ei.source_facility_id, 
        ei.edge_count_from_source, ei.mode
         from edges ei, vertices vi
        where vi.vertex_id = ei.d_vertex_id
         and edge_type = 'transport'
         and ifnull(distance_travelled, 0) > 0
        ...
        for row_a in source_edge_data:
            ...
            to_vertex = row_a[17]
            previous_distance_travelled = row_a[18]
            ...
            distance_travelled = previous_distance_travelled + length
                            ...
                            if from_location != 'NULL' and to_location == 'NULL':
                                ...
                                main_db_con.execute("""insert or ignore into edges (from_node_id, to_node_id,
                                    ...
                                    """.format(from_node, to_node,
                                               ...
                                               source_facility_id,
                                               distance_travelled, 'N', edge_count_from_source, total_route_cost))
...
def generate_all_edges_from_source_facilities(the_scenario, schedule_length, logger):
    """
    Iteratively extend transport edges for commodities with max transport distances.

    This function continuously adds child edges to existing transport edges until destination
    facilities are reached or the maximum commodity transport distance is exceeded. It manages
    endcap nodes where transport terminates due to distance limits.

    :param the_scenario: The configuration object containing settings and paths.
    :param schedule_length: Integer length of the simulation schedule.
    :param logger: The logging object.
    :return: None
    :db_reads: edges, commodity_mode, networkx_edges, networkx_edge_costs, source_commodity_ref, networkx_nodes, vertices, pipeline_mapping
    :db_writes: edges, endcap_nodes, edge_index (index)
    """
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        while edges_requiring_children > 0:
            ...
            # the shortest -- if there is a valid nx_edge to build, the crossing node is not an endcap if total length
            ...
            (select count(edge_id) parents,
             min(distance_travelled) leadin_distance_travelled,
             *
            from edges
            where children_created = 'N'
            -----------------do not mess with this "group by"
            group by to_node_id, source_facility_id, commodity_id, end_day
            ------------------it affects which columns we're checking over for min distance travelled
            ...
            # if we're trying to add an edge that has an entry in route_data, new distance travelled must be less
            ...
            for row_a in potential_edge_data:
                ...
                vertex_id = row_a[15]
                source_facility_id = row_a[16]
                leadin_edge_distance_travelled = row_a[17]
                ...
                if mode in the_scenario.permittedModes and (mode, commodity_id) in commodity_mode_dict.keys() \
                    ...
                    # create new edge
                    elif new_distance_travelled <= max_commodity_travel_distance:
                                ...
                                if simple_mode != 'pipeline' or tariff_id >= 0:
                                    ...
                                    # overwritten if we've found a shorter path to a previous endcap
                                        ...
                                        main_db_con.execute("""insert or ignore into edges (from_node_id, to_node_id,
                                                ...
                                                phase_of_matter,
                                                source_facility_id,
                                                distance_travelled, children_created, edge_count_from_source, 
                                                total_route_cost) 
                                                VALUES ({}, {},
                                                {}, {}, {},
                                                {}, {},
...
def generate_all_edges_without_max_commodity_constraint(the_scenario, schedule_length, logger):
    """
    Generate transport edges for commodities without a maximum transport distance constraint.

    Identifies the shortest paths (using the `shortest_edges` table pre-calculated via NetworkX)
    and creates transport edges for all viable commodities and days that match permitted modes.
    Unlike source-based generation, this creates edges globally across the shortest path network.

    :param the_scenario: The configuration object containing settings and paths.
    :param schedule_length: Integer length of the simulation schedule.
    :param logger: The logging object.
    :return: None
    :db_reads: commodity_mode, networkx_edges, shortest_edges, networkx_nodes, networkx_edge_costs, source_commodity_ref, vertices, pipeline_mapping, facility_commodities
    :db_writes: edges, edge_index (index)
    """
...
def generate_edges_from_routes(the_scenario, schedule_length, logger):
    """
    Generate transport edges based on pre-defined OD pairs and routes.

    This method is typically used when network density reduction (NDR) is active.
    It creates transport edges corresponding to specific routes identified in
    the `route_reference` table, populated from `od_pairs` and `route_edges`.

    :param the_scenario: The configuration object containing settings and paths.
    :param schedule_length: Integer length of the simulation schedule.
    :param logger: The logging object.
    :return: None
    :db_reads: od_pairs, route_edges, networkx_edges, networkx_edge_costs, networkx_nodes, vertices, facility_commodities
    :db_writes: route_reference, edges
    """
            ...
            for day in range(1, schedule_length+1):
                if day + fixed_route_duration <= schedule_length:
                    ...
                    main_db_con.execute("""insert or ignore into edges (route_id, from_node_id, 
                                to_node_id, start_day, end_day, commodity_id, o_vertex_id, d_vertex_id, 
                                edge_flow_cost, edge_type, 
                                length,phase_of_matter, source_facility_id) VALUES ({}, {}, {}, {}, {}, {}, {}, {}, {}, 
                                '{}', {},'{}', {}
                                )""".format(route_id,
                                            from_node_id,
                                            to_node_id,
...
def set_edges_volume_capacity(the_scenario, logger):
    """
    Populate volume, capacity, and unit conversion data for edges.

    Updates the `edges` table with:
    1. Base volume and capacity for road/rail/water edges from `networkx_edges`.
    2. Specific capacity for pipeline edges from `pipeline_mapping` and `capacity_nodes`.
    3. Lock capacities for water routes.
    4. Unit conversion multipliers (e.g., solid vs liquid) based on mode and phase of matter.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object.
    :return: None
    :db_reads: networkx_edges, pipeline_mapping, capacity_nodes, networkx_nodes
    :db_writes: edges (updates)
    """
...
def pre_setup_pulp(logger, the_scenario):
    """
    Orchestrate the setup of the optimization graph and data structures.

    This meta-function calls sub-functions to:
    1. Setup source tracking.
    2. Generate schedules.
    3. Create vertices for facilities.
    4. Create edges (storage, connectors, and transport).
    5. Set volume and capacity limits on edges.
...
def create_flow_vars(the_scenario, logger):
    """
    Create PuLP variables representing flow along edges.

    Reads the `edges` table and creates a dictionary of PuLP variables for each edge.
    These variables represent the quantity of commodity flowing on that edge.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object.
    :return: A dictionary of `LpVariable` objects, keyed by edge_id.
    :db_reads: edges
    """
...
def create_unmet_demand_vars(the_scenario, logger):
    """
    Create PuLP variables representing unmet demand at ultimate destinations.

    Reads facility demand data and creates variables that effectively act as slack
    in the optimization, allowing the solver to penalize but not fail if demand is not met.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object.
    :return: A dictionary of `LpVariable` objects, keyed by (facility_id, schedule_day, commodity_name, udp_cost).
    :db_reads: vertices, commodities, facility_type_id, facilities
    """
...
def create_candidate_processor_build_vars(the_scenario, logger):
    """
    Create binary PuLP variables for the decision to build candidate processors.

    Reads facilities marked as 'candidate' processors and creates binary variables
    that trigger build costs if set to 1.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object.
    :return: A dictionary of `LpVariable` objects (Binary), keyed by facility_id.
    :db_reads: facilities, facility_type_id
    """
...
def create_binary_processor_vertex_flow_vars(the_scenario, logger):
    """
    Create binary PuLP variables tracking if a processor is active on a given day.

    These variables are used to enforce minimum/maximum capacity constraints on
    processors. If active (1), flow constraints apply.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object.
    :return: A dictionary of `LpVariable` objects (Binary), keyed by (facility_id, schedule_day).
    :db_reads: vertices, facility_type_id
    """
...
def create_processor_excess_output_vars(the_scenario, logger):
    """
    Create PuLP variables for excess material output at processors.

    Allows the optimization to discard or "store" excess material produced by a processor
    that cannot be immediately moved downstream.

    :param the_scenario: The configuration object containing settings and paths.
    :param logger: The logging object.
    :return: A dictionary of `LpVariable` objects, keyed by vertex_id.
    :db_reads: vertices, facility_type_id
    """
...
def create_opt_problem(logger, the_scenario, unmet_demand_vars, flow_vars, processor_build_vars):
    """
    Initialize the PuLP optimization problem and define the Objective Function.

    The objective is to Minimize Cost. Cost components include:
    1. Unmet demand penalties.
    2. Flow costs (transportation and storage).
    3. Processor build costs (for candidate facilities).

    :param logger: The logging object.
    :param the_scenario: The configuration object.
    :param unmet_demand_vars: Dictionary of unmet demand variables.
    :param flow_vars: Dictionary of edge flow variables.
    :param processor_build_vars: Dictionary of binary build variables.
    :return: The initialized `LpProblem` object with the objective function set.
    :db_reads: edges, candidate_processors, candidate_process_list, facilities, facility_type_id
    """
...
def create_constraint_unmet_demand(logger, the_scenario, prob, flow_var, unmet_demand_var):
    """
    Generate constraints for unmet demand at Ultimate Destinations.

    Enforces that for every destination facility, day, and commodity:
    (Sum of Inbound Flow) + (Unmet Demand Variable) == (Total Required Demand).

    :param logger: The logging object.
    :param the_scenario: The configuration object.
    :param prob: The `LpProblem` object to add constraints to.
    :param flow_var: Dictionary of edge flow variables.
    :param unmet_demand_var: Dictionary of unmet demand variables.
    :return: The modified `LpProblem` object.
    :db_reads: vertices, commodities, facility_type_id, facilities, edges
    """
...
def create_constraint_max_flow_out_of_supply_vertex(logger, the_scenario, prob, flow_var):
    """
    Generate constraints limiting flow out of Raw Material Producers (RMPs).

    Enforces that the sum of flow leaving a supply node does not exceed its
    defined supply capacity (Activity Level * Max Daily Supply).
    ...
    :param flow_var: Dictionary of edge flow variables.
    :return: The modified `LpProblem` object.
    :db_reads: vertices, facility_type_id, edges
    """
...
def create_constraint_daily_processor_capacity(logger, the_scenario, prob, flow_var, processor_build_vars,
    ...
    """
    Generate constraints for processor daily minimum and maximum capacities.

    1. Enforces min/max flow into a processor if it is active.
    2. Links flow activity to the binary build decision for candidate processors.
    (If flow > 0, then the processor must be 'built').
    ...
    :param flow_var: Dictionary of edge flow variables.
    :param processor_build_vars: Dictionary of binary build variables.
    :param processor_daily_flow_vars: Dictionary of binary variables for daily activation.
    :return: The modified `LpProblem` object.
    :db_reads: facility_commodities, facility_type_id, facilities, vertices, edges
    """
...
def create_primary_processor_vertex_constraints(logger, the_scenario, prob, flow_var):
    """
    Generate constraints for processor conversion ratios and conservation.

    Enforces that:
    1. Input commodity quantity aligns with output commodity quantity based on the
       defined process ratios (from `facility_commodities`).
    2. Input/Output flows match regarding source tracking if required.
    3. If there is no input, there can be no output.
    ...
    :param flow_var: Dictionary of edge flow variables.
    :return: The modified `LpProblem` object.
    :db_reads: vertices, facility_commodities, facility_type_id, commodities, facilities, edges
    """
...
def create_constraint_conservation_of_flow(logger, the_scenario, prob, flow_var, processor_excess_vars):
    """
    Generate standard Conservation of Flow constraints for storage and network nodes.

    Enforces Flow In == Flow Out for:
    1. Storage vertices (including accounting for processor excess material).
    2. NetworkX nodes (intersections in the transportation graph).
    ...
    :return: The modified `LpProblem` object.
    :db_reads: vertices, facility_type_id, commodities, facilities, edges, networkx_nodes
    """
...
def create_constraint_max_route_capacity(logger, the_scenario, prob, flow_var):
    """
    Generate capacity constraints for storage and transportation routes (Road, Rail, Water).

    Enforces that:
    1. Flow into storage does not exceed storage capacity.
    2. Flow on road/rail/water links does not exceed vehicle/link capacity.
    Handles background flows and minimum capacity levels if configured.
    ...
    :param flow_var: Dictionary of edge flow variables.
    :return: The modified `LpProblem` object.
    :db_reads: route_reference, storage_routes, edges, commodities
    """
...
def create_constraint_pipeline_capacity(logger, the_scenario, prob, flow_var):
    """
    Generate capacity constraints specifically for pipeline tariffs.

    Groups flow by 'link_id' (which aggregates multiple tariff definitions for the same physical
    pipeline segment) to ensure total flow does not exceed physical capacity.
    ...
    :param flow_var: Dictionary of edge flow variables.
    :return: The modified `LpProblem` object.
    :db_reads: edges, pipeline_mapping, capacity_nodes, commodities
    """
...
def setup_pulp_problem(the_scenario, logger):
    """
    Assemble the full optimization problem structure.

    Calls sub-functions to create all variables, defining the objective function,
    and applying all necessary constraints. Finally, writes the problem to an LP file
    for debugging.

    :param the_scenario: The configuration object.
    :param logger: The logging object.
    :return: The fully constructed `LpProblem` object ready for solving.
    """
...
def solve_pulp_problem(prob_final, the_scenario, logger):
    """
    Execute the solver (CBC or HiGHS) on the defined optimization problem.

    Accesses user-specified time_limit from the scenario object.

    :param prob_final: The fully constructed `LpProblem`.
    :param the_scenario: The configuration object containing solver settings.
    :param logger: The logging object.
    :return: The solved `LpProblem` object (containing the solution state).
    """
...
def save_pulp_solution(the_scenario, prob, logger, zero_threshold):
    """
    Extract nonzero variable values from the solution and save to the database.

    Creates the `optimal_solution` table and populates it with variable names
    and values that exceed the zero threshold. Also logs key metrics like
    total unmet demand penalty.

    :param the_scenario: The configuration object.
    :param prob: The solved `LpProblem` object.
    :param logger: The logging object.
    :param zero_threshold: Float threshold below which values are treated as zero.
    :return: None
    :db_writes: optimal_solution
    """
    import datetime
    start_time = datetime.datetime.now()
    ...
    with sqlite3.connect(the_scenario.main_db) as db_con:
        ...
        if optimal_solution_rows:
            db_con.executemany(
                "insert into optimal_solution (variable_name, variable_value) values (?, ?);",
                optimal_solution_rows,
            )
            non_zero_variable_count = len(optimal_solution_rows)
        ...
        for row in customized_udp_data:
            ...
            sql = """SELECT f.facility_name FROM facilities f join facility_type_id fti on f.facility_type_id = fti.facility_type_id WHERE f.facility_ID = ? and fti.facility_type = 'ultimate_destination'"""
            facility_name_row = db_cur.execute(sql, (facility_id,)).fetchone()
            facility_name = facility_name_row[0] if facility_name_row else f"facility_id={facility_id}"
...
def record_pulp_solution(the_scenario, logger):
    """
    Transform raw solution variables into a structured database table.

    Parses variable names from `optimal_solution` (e.g., 'Edge_123') to link back
    to meaningful entities (facilities, commodities, time periods) and stores
    the result in `optimal_variables`. Also handles unit reconversion (solid vs liquid).

    :param the_scenario: The configuration object.
    :param logger: The logging object.
    :return: None
    :db_reads: optimal_solution, edges, commodities, vertices, source_commodity_ref
    :db_writes: optimal_variables
    """
    ...
    with sqlite3.connect(the_scenario.main_db) as db_con:
        ...
        sql = """
            ...
            null as final_facility,
            null as prior_edge,
            edges.distance_travelled as distance_travelled
            from optimal_solution
...
def parse_optimal_solution_db(the_scenario, logger):
    """
    Read optimal solution from database into Python data structures.

    This function loads results into memory for inspection or further processing without raw SQL queries.
    Called in P and OC steps but only used in OC step.

    :param the_scenario: The configuration object.
    :param logger: The logging object.
    :return: A tuple containing:
             - optimal_processors (list)
             - optimal_route_flows (dict)
             - optimal_unmet_demand (dict)
             - optimal_storage_flows (dict)
             - optimal_excess_material (dict)
    :db_reads: optimal_solution, edges, vertices
    """
