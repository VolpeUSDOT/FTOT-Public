...
# For non-earthquake occurrence scenario, yearly basis simulation is utilized
# ---------------------------------------------------------------------------------------------------
...
# load facility capacity (2-D matrix)
# row: facility ID, column: facility capacity index for each week
facility_time_varying_cap = np.load("facility_time_varying_cap.npy")

# load repair cost array (1-D matrix): repair costs of damaged facilities at each week
repair_costs = np.load("repair_costs.npy")

# Load edge capacity (2-D matrix)
# row: edge ID, column: edge capacity index for each week
edge_time_varying_cap = np.load("edge_time_varying_cap.npy")

# weekly index
weekly_index = float(7)/float(365)

# Unit operation cost
operation_cost = 5790 # per Kgal
#-----------------------------------------------------------------------------
# Unmet demand threshold, used to calculate the increased unmet demand due to earthquake
UD_level = 4141200
# Total final demand threshold
...
# These threshold are yearly value

# Weeks in one year
WeekInYear = 52 # used to transfer the yearly threhold values to weekly
            ...
            if facility_type == "processor":
               ...
               and mc.commodity_name = '{}';""".format(facility_id, multi_commodity_name))
                ...
                for row_b in proc_data:
                    ...
                    quantity = facility_time_varying_cap[float(facility_id)-1][j]*row_b[1]*weekly_index
                   
                    # Since the value in facility CSV table is the yearly value, "weekly_index" is applied to transfer the quantity to weekly value
                    #=========================================================================================================================
                ...
                for row_b in dest_data:
                    commodity_id = row_b[0]
                    quantity = row_b[1]*weekly_index # daily_index is used to transfer the yearly value in facility CSV file into weekly value
...
def create_unmet_demand_vars(the_scenario, logger):
    ...
    unmet_demand_var = LpVariable.dicts("UnmetDemand", demand_var_list, None, None)# lower bound of undemt demand is not 0. 
...
def create_constraint_max_route_capacity(logger, the_scenario, prob, flow_var):
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        for row_a in route_capac_data:
            ...
            capacity_reduction_index = edge_cap[float(edge_id)-1][j]
...
def save_pulp_solution(the_scenario, prob, logger, zero_threshold=0.00001):
    ...
    with sqlite3.connect(the_scenario.main_db) as db_con:
        ...
        unmet_demand_weekly = optimal_unmet_demand_sum -(float(UD_level)/float(WeekInYear))
        np.save("unmet_demand_weekly.npy", unmet_demand_weekly)
        
        production = float(total_final_demand)/float(WeekInYear) - optimal_unmet_demand_sum
        np.save("production.npy", production)
        
        logger.info("production : {}".format(production))    
        logger.info("Total Unmet Demand : {}".format(unmet_demand_weekly))
        logger.info("Repair Cost : {}".format(repair_costs[j]))
        ...
        if unmet_demand_weekly < 0:
            costs_weekly = abs(float(value(prob.objective))- optimal_unmet_demand_sum * the_scenario.unMetDemandPenalty) + repair_costs[j] + operation_cost*production

            logger.result(
                "Total Scenario Cost = (transportation + processor construction + restoration +operation): \t ${0:,.0f}"
                "".format(costs_weekly))
        else:
            costs_weekly = abs(float(value(prob.objective))-optimal_unmet_demand_sum * the_scenario.unMetDemandPenalty)+ repair_costs[j] + operation_cost*production + unmet_demand_weekly*the_scenario.unMetDemandPenalty
            logger.result(
                "Total Scenario Cost = (transportation + unmet demand penalty + processor construction + restoration + operation): \t ${0:,.0f}"
                "".format(costs_weekly))
            
        np.save("costs_weekly.npy", costs_weekly)
 
        logger.info("costs_weekly: {}".format(costs_weekly))
        #=======================================================================================
