...
Operation_unitcost = 0.00579 # unit:$/kgal (2021 dollars)
total_final_demand = 4073312.9 + 254401.1
Inflation_rate = 0.0279
# daily index
            ...
            if facility_type == "processor":
               ...
               and mc.commodity_name = '{}';""".format(facility_id, multi_commodity_name))
                ...
                for row_b in proc_data:
                    ...
                    temp_day = int(7*j)
                    if temp_day < repair_time_facility[float(facility_id)-1][t+1][i]:
                        quantity = facility_cap[float(facility_id)-1][t+1][i]*row_b[1]*daily_index
                    else:
                        quantity = facility_cap_noEarthquake[float(facility_id)-1][t+1][i]*row_b[1]*daily_index
                    #=============================================================================================
                    io = row_b[4]
...
def create_opt_problem(logger, the_scenario, unmet_demand_vars, flow_vars, processor_build_vars):
    ...
    for u in unmet_demand_vars:
        ...
        operation_costs.append(Operation_unitcost * (total_final_demand - unmet_demand_vars[u]))
    ...
    prob += (lpSum(unmet_demand_costs) + lpSum(operation_costs)+ lpSum(flow_costs[k] * flow_vars[k] for k in flow_costs) + lpSum(
...
def create_constraint_daily_processor_capacity(logger, the_scenario, prob, flow_var, processor_build_vars,
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        for row_a in processor_facilities:
            ...
            temp_day = int(7*j)
            if temp_day < repair_time_facility[float(facility_id)-1][t+1][i]:
                max_capacity = facility_cap[float(facility_id)-1][t+1][i]*row_a[2]*daily_index
            else:
                max_capacity = facility_cap_noEarthquake[float(facility_id)-1][t+1][i]*row_a[2]*daily_index
            #-----------------------------------------------------------------------------------------------
...
def create_primary_processor_vertex_constraints(logger, the_scenario, prob, flow_var):
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        for row_a in sql_data:
            ...
            # nx_edge_id = row_a[6]
            facility_id = row_a[8]
            quantity = facility_cap_noEarthquake[float(facility_id)-1][t+1][i]*float(row_a[7])
...
def create_constraint_max_route_capacity(logger, the_scenario, prob, flow_var):
    ...
    with sqlite3.connect(the_scenario.main_db) as main_db_con:
        ...
        sql = """select e.edge_id, e.commodity_id, e.nx_edge_id, e.max_edge_capacity, e.start_day, e.simple_mode, e.phase_of_matter,
        ...
        for row_a in route_capac_data:
            ...
            commodity_id = row_a[1]
            nx_edge_id = row_a[2]
            nx_edge_capacity = row_a[3]
            start_day = row_a[4]
            simple_mode = row_a[5]
            phase_of_matter = row_a[6]
            capac_minus_background_flow = max(row_a[7], 0)
            ...
            temp_day = int(7*j)
            if float(edge_id) in repair_time_edge[:,1,1]:
            ...
            flow_lists.setdefault((nx_edge_id, converted_capacity, commodity_id, start_day), []).append(flow_var[edge_id])

        for key, flow in iteritems(flow_lists):
            prob += lpSum(flow) <= key[1], "constraint max flow on nx edge {} commodity_id {} for day {}".format(key[0], key[2], key[3])
...
def save_pulp_solution(the_scenario, prob, logger, zero_threshold=0.00001):
    ...
    with sqlite3.connect(the_scenario.main_db) as db_con:
        ...
        if optimal_unmet_demand_sum < 0:
            costs_daily = (abs(float(value(prob.objective))- optimal_unmet_demand_sum * the_scenario.unMetDemandPenalty))*((1+Inflation_rate)**t) + sum(CatalystReplace_cost[:,t+1,i])*daily_index  + repair_costs[int(j*7)][t][i]

            logger.result(
                "Total Scenario Cost = (transportation + processor construction + operation + catalyst replacement): \t ${0:,.0f}"
                "".format(costs_daily))
        else:
            costs_daily = (abs(float(value(prob.objective))-optimal_unmet_demand_sum * the_scenario.unMetDemandPenalty))*((1+Inflation_rate)**t)+sum(CatalystReplace_cost[:,t+1,i])*daily_index + repair_costs[int(j*7)][t][i] + (optimal_unmet_demand_sum*the_scenario.unMetDemandPenalty)*((1+Inflation_rate)**t)
            logger.result(
